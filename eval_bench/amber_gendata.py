...
from amber_loader_gencoco import AMBERDataSet

# import kornia
from transformers import set_seed
from avisc_utils.vcd_add_noise import add_diffusion_noise
from avisc_utils.avisc_sample_gendata import evolve_avisc_sampling
...
def parse_args():
    ...
    parser.add_argument("--checkpoint_path", type=str, default="/home/zlj/Octopus/checkpoint/try")
...
#         out_transformer = self.transformer(inputs_transformer,hidden_states)
...
def main():
    ...
    print("Start eval...")
    result_json_path = os.path.join(args.checkpoint_path, "gendata_coco.json")
    ...
    for batch_id, data in tqdm(enumerate(AMBER_loader), total=len(AMBER_loader)):
        ...
        for i in range(10):
            ...
            for id,id1, a in zip(ids,idt,outputs):

                response.append(a)
                action.append(action_scores.tolist()[0])
                if i==9:
                    item = {
                        "id": int(id),
                        "id_t": int(id1),
